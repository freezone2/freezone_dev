                    ...
                    foreach ( $current_items as $mmm => $arItem) {
                      ...
                      <div class="index-service-item <?=($flights >= 10 ? 'min10' : 'min2');?>"
                            ...
                            <div class="index-service-item-drop">
                                ...
                                <div class="index-service-item-bottom"><?=round ( $price ['PRICE'] );?>.-</div>
                                <button data-id="<?=$arItem ['ID'];?>" class="get_tariff button btn-white"><?=Loc::getMessage("ORDER_CERTIFICATE");?></button>
                            </div>
                        </div>
                        <?
                        /* if ($item == 3) {
...
if (!empty($_GET['tid'])) {
    ...
    <script>
        setTimeout(function(){
            $('.cart-list .index-service-item button.get_tariff[data-id="<?=$tid;?>"]').click();
        }, 500);
    </script>
    <?
}

...
function get_truba_time_settings($truba_id, $current_day)
{
  ...
  $arRes = array();
  $arFilter = array(
    'IBLOCK_ID' => 61, // ID инфоблока настроек времени
    'ACTIVE' => 'Y',
  ...
  $prop = CIBlockPropertyEnum::GetList(array(),array('IBLOCK_ID' => 61, 'CODE' => 'WEEK', 'XML_ID'=>$number_day));
  ...
  $res = CIBlockElement::GetList(
    ...
    array(
      ...
      'PROPERTY_TIME_FROM', 'PROPERTY_TIME_TO', 'PROPERTY_TUBE', 'PROPERTY_WEEK',
    )
  );
    ...
    do {
        ...
        if ($reserv_data === false) {
            ...
            if (!($hide_busy && !$is_need_more_people)) {
                ...
                if ($filter_person_type == PERSONE_TYPE_PROF || $is_need_more_people) {
                    $data_content_times = $data_timename;
                }
            ...
            if (sizeof($array_times)) {
                ...
                foreach ($array_times as $addon) {
                    ...
                    <li
                        ...
                        <? if ($data_datename){ ?>data-datename="<?= $data_datename; ?>"<?
                    } ?>
...
function create_order($params, $use_event = true)
{
    ...
    if($params['ORDER_DAY'] && $params['ORDER_TIME']){
      ...
      $date = FormatDate('j F Y', strtotime(date()));
...
function get_tariff($tariff_id)
{
    if ($tariff_id) {
        $res = CIBlockElement::GetList(false, array('IBLOCK_ID' => 42, 'ID' => $tariff_id), 0, 0, array(
            ...
            'PROPERTY_BLOG_POST_ID', 'PROPERTY_BLOG_COMMENTS_CNT', 'TRUBA',
...
function build_week($current_timestamp = null, $params = array())
{
    ...
    $new_day = false;
    for ($i = $sh; $i < $sh + 24; $i++) {
        $content .= '<tr><th style="height: 96px">' . $sh_c . '</th></tr>';
        $eh_c = $sh_c + 1;
            ...
            for ($lnum = 1; $lnum <= 4; $lnum++) {
                ...
                if (!empty($order_raw['ID'])) {
                    ...
                    if ($length == 2) {
                        ...
                        ';
                        if ($prev_id == $order['ID']) {
                            $block_content = '<p class="' . $class_block . '"><span>' . $start_h . ':' . $start_m . '</span></p>';
                        }
                ...
                $content .= '
                    <li ' . (!empty($order_raw['ID']) ? 'data-order_id="' . $order['ID'] . '" 
                        class="order_bl order_bl' . $order['ID'] . '"  
                        data-class="order_bl' . $order['ID'] . '"
                        id="current_bl' . $order['ID'] . '_' . $lnum . '"' : '') . '>' . $block_content . '</li>
                ';
                $start_h = (int)$end_h;
                $start_m = (int)$end_m;
            }

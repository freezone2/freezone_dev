...
if ($USER->IsAuthorized() || $arParams["ALLOW_AUTO_REGISTER"] == "Y" )
{
	if(strlen($_REQUEST["ORDER_ID"]) <= 0)
	{
		...
		if (empty($arResult["BASKET_ITEMS"]) || !is_array($arResult["BASKET_ITEMS"]))
		{
			...
			{
                ?>
                <div style="text-align: center">Ваша корзина пуста. Пожалуйста, <a href="/menu/">выберете товар.</a></div>
        ...
        if (empty($_POST["ORDER_PROP_4"])) {
           // $_POST["PAY_SYSTEM_ID"] = 2;
            $_POST["ORDER_PROP_4"] = 1;
        }
